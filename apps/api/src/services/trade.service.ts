...
function createTradeRequestHash(input: Record<string, string>) {
...
async function getStoredIdempotentTradeResult(
  userId: string,
  key: string,
  requestHash: string,
) {
  const existing = await prisma.tradeIdempotencyKey.findUnique({
    where: {
      userId_key: {
        userId,
        key,
  ...
  if (existing.response === null) {
    throw new AppError("Trade request is already processing", 409);
  }
...
async function runIdempotentTrade<T>(
  userId: string,
  key: string,
  requestHash: string,
  callback: (tx: Prisma.TransactionClient) => Promise<T>,
) {
  const storedResult = await getStoredIdempotentTradeResult(
    userId,
    key,
    requestHash,
  );
  ...
  try {
    const result = await runSerializableTrade(async (tx) => {
      await tx.tradeIdempotencyKey.create({
        data: {
          userId,
          key,
          requestHash,
      ...
      await tx.tradeIdempotencyKey.update({
        where: {
          userId_key: {
            userId,
            key,
    ...
    if (isUniqueConstraintConflict(error)) {
      const existingResult = await getStoredIdempotentTradeResult(
        userId,
        key,
        requestHash,
      );
...
export async function buyCoin(
  ...
  const requestHash = createTradeRequestHash({
    type: "BUY",
    symbol,
    usdAmount: grossUsd.toString(),
  });

  const { result, replayed } = await runIdempotentTrade(
    userId,
    idempotencyKey,
    requestHash,
    async (tx) => {
      ...
      if (!coin.isActive) {
        throw new AppError(
          "This coin is currently unavailable for trading",
          400,
        );
      }
...
export async function sellCoin(
  ...
  const requestHash = createTradeRequestHash({
    type: "SELL",
    symbol,
    coinAmount: coinAmountToSell.toString(),
  });
